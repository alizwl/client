...
void PhotoViewController::viewDidLoad()
{
	...
	btn->addTarget(this, CAControl_selector(PhotoViewController::buttonCallBack), CAControlEventTouchUpInSide);
...
void PhotoViewController::requestMsg() // all(0) track(1) format(2) my_all(3)
{
	if (m_alertView) {
	...
	CommonHttpManager::getInstance()->send_post(httpUrl, key_value, this, CommonHttpJson_selector(PhotoViewController::onRequestFinished));
}
...
void PhotoViewController::showAlert()
{
	if (m_alertView) {
		this->getView()->removeSubview(m_alertView);
		m_alertView = NULL;
	...
	btn5->addTarget(this, CAControl_selector(PhotoViewController::buttonCallBack), CAControlEventTouchUpInSide);
